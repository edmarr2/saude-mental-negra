...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if no password confirmation is diferent of password', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'edmar',
  ...
  test('Should return with error 500 if EmailValidator throws', async () => {
    const { sut, emailValidatorStub } = makeSut()
  ...
  test('Should call AddAccount with correct values', async () => {
    ...
    const addAccountSpy = jest.spyOn(addAccountStub, 'add').mockImplementation(() => { throw new Error() })
    const httpRequest = {
      body: {
        name: 'edmar',
        email: 'invalid_email@mail.com',
